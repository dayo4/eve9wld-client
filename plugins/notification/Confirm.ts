...
interface ConfirmInteface {
    ...
    message: string,
    type?: string,
    onConfirm: Function /* "onConfirm" is the function to trigger when user confirms the action. It should return a value(promise) if it's an async operation */
}
    ...
    div.innerHTML = `
        <div class="Content m-auto xs11 sm8 md6 lg5">
            ...
            <div class="Foot bg-white noselect flex a-i-center j-c-center p-2">
                ...
                > Cancel </button>
                <button id="ApproveActionVerification"
                    class="ApproveActionVerification btn pink-gradient-btn icon-hammer"
                > Confirm </button>
            </div>
