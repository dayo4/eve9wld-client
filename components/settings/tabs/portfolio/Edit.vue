<template>
  <div>
    <h2 class="text-center t-blue-grey--1">
      {{ projectToView ? "EDIT" : "NEW" }} PORTFOLIO PROJECT
    </h2>

    <h3
      class="flex a-i-center shadow-1 j-c-between bg-pink-1 t-white font-7 p-2 noselect br2"
    >
      <span class="font-5">{{
        projectToView ? "EDIT PROJECT" : "CREATE NEW"
      }}</span>
    ...
    <section v-if="projectToView">
      ...
      <span class="icon-info bg-blue-5 p-3"
        >Only active projects will be visible in your profile. New projects are
    ...
    <section>
      ...
      <span v-if="errors && errors['Title']" class="t-red p-1 my-1">{{
        errors["Title"]
      }}</span>
      <div
        ref="titleInput"
    ...
    <section>
      ...
      <div class="icon-info bg-blue-5 p-3 mb-2">
        Full detail of the project or work experience. You can edit or design as
        much as you want taking advantage of the editor. You can embed link to
        videos but images will be discarded.
      </div>
      <span v-if="errors && errors['Detail']" class="t-red p-1 my-1">{{
        errors["Detail"]
      }}</span>
      <!-- editor -->
      <TextEditor
        @contentUpdated="setDetail"
        :initialContent="initialDetail"
      ></TextEditor>
    ...
    <div class="flex j-c-between">
      <div class="flex">
        <button @click="saveChanges" class="bg-pink--3 mt-4 mr-5 btn">
          Save Project
        </button>
...
export default Vue.extend({
  components: {
    // TextEditor: () =>
    //   import("@/components/GlobalComponents/utils/TextEditor.vue"),
  ...
  data() {
    return {
      initialDetail: this.projectToView
        ? this.projectToView.detail
        : ("" as string) /* Only useful when viewing existing project detail */,
      title: "",
      detail: "",
  ...
  mounted() {
    if (this.projectToView) {
      ...
      this.title = (this.$refs
        .titleInput as HTMLDivElement).textContent = this.projectToView.title;
    }
  }
});
</script>
