<template>
  <Modal
    ...
    @close="close"
    @action="clear"
    RCB="icon-trash-empty"
  >
    <!-- heading -->
    <template v-slot:heading
      >Your Reading Queue ({{ queue ? queue.length : 0 }})</template
    >
    ...
    <template v-slot:default>
      ...
      <section v-show="!queue || queue.length === 0" class="text-center">
        ...
        <p class="t-grey--4">
          Use the "Add to queue" option on posts to queue up your favourite
          contents for viewing later.
        </p>
