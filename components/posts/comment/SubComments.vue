<template>
  ...
  >
    ...
    <section class="SourceWrapper br2 mb-3 mr-2 mt-10 px-2">
      ...
      >
        <div class="bg-blue-grey-4 shadow-">
          {{
            activeComment.user.first_name + " " + activeComment.user.last_name
          }}
        </div>
      </div>
    ...
    <section v-else>
      <h2 class="t-blue-grey text-center text-cap font-7 mt-10 letter-space-1">
        Reply to
        {{ activeComment.user.first_name + " " + activeComment.user.last_name }}
      </h2>
    </section>
...
import { $Comments, $Auth } from "@/store";
