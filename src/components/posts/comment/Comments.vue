            ...
            >
              ...
              <div class="shadow-2 bg-grey-5 br3">
                ...
                <div class="Actions font-3 t-grey bold-2 p-2 noselect">
                  <span @click="replies(comment, true)" class="icon-reply"
                    >Reply</span
                  >
...
import { $Comments, $Posts, $Auth } from "@/store";
import { $Validator, $Obstacl, WS } from "@/plugins";

export default Vue.extend({
  components: {
    Minimizer: () =>
      import("@/components/GlobalComponents/utils/Minimizer.vue"),
  ...
  props: {
    ...
    post: {
      required: true
    } as any /* current post which's comment is being viewed */,
    socket: {
      required: true /* SocketIOClient.Socket  */
    } as any /* An instance of the comments socket connection */
  },
  ...
  methods: {
    ...
    input(event) {
      // console.log(event)
      if (this.user) this.commentContent = event.target.textContent;
    },
    ...
    scrollUp() {
      if (this.subCommentMode === true)
        document
          .getElementById("SubCommentBody")
          .scrollTo({ top: 0, left: 0, behavior: "smooth" });
      else
        (this.$refs.CommentBody as HTMLDivElement).scrollTo({
          top: 0,
          left: 0,
          behavior: "smooth"
        });
