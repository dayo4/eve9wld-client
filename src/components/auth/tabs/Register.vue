<template>
  <div v-show="show" class="GenFormWrapper py-4 shadow-12 bg-cyan--3">
    <div
      class="GenFormStyle shadow-12 bg-white py-8 px-5"
      @keydown.enter="send"
    >
      <h3
      ...
      <div class="Section">
        ...
        <span class="Error t-red-1" v-show="username_err">{{
          username_err
        }}</span>
        <input
          v-model="username"
      ...
      <transition name="expand">
        <div v-if="response.message" class="mt-5">
          <div
            :class="
              response.error ? 'bg-pink-5 t-red-1' : 'bg-lime-4 t-green--1'
            "
            class="Alert p-3 b1"
          >
            {{ response.message }}
      ...
      const schema = [
        {
          ...
          messages: {
            pattern:
              "Username may only contain letters and numbers. Must start with minimum of 3 letters"
          }
        },
        {
          fieldName: "Email",
