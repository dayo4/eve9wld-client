  ...
  <transition name="slide-fade">
    <div class="AuthModal noselect" v-if="showModal">
      ...
      <transition name="slide-fade">
        ...
        <h4
          class="bg-light-blue-5 t-light-blue p-4 my-0"
          v-if="query.showQuery"
        >
          <span class="icon-info mr-2 t-blue"></span>
          {{
            query.message
              ? query.message
              : "Sorry! You have to login to access this section."
          }}
        </h4>
      </transition>
      ...
      <div class="FormCont flex j-c-center bg-white h-full">
        <div class="Pillar xs11 sm9 md7 lg5">
          ...
          <div v-if="mode != 1" class="flex wrap a-i-center mb-5 t-blue-grey-1">
            <span class="mr-3 bold-2">{{
              mode === 3 ? "Oh! I just remembered" : "Already have an account?"
            }}</span>
            <span
              @click="switchMode(1)"
