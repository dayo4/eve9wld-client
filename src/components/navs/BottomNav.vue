...
export default Vue.extend({
  ...
  watch: {
    $route() {
      if (this.$route.path === "/") this.homeRoute = true;
      else this.homeRoute = false;

      if (history.length > 0) this.backable = true;
      else this.backable = false;
    }
  },
  ...
  methods: {
    scrollUp() {
      if ($Navs.$container.scrolled)
        $Navs.$container.element.scrollTo({
          top: 0,
          left: 0,
          behavior: "smooth"
        });
